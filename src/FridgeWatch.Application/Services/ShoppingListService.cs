...
public class ShoppingListService : IShoppingListService
{
    ...
    {
        var shoppingList = (await _unitOfWork.ShoppingLists.GetWithItemsByIdAsync(id)).EnsureExists("采购清单不存在");
    ...
    {
        var shoppingList = (await _unitOfWork.ShoppingLists.GetByIdAsync(id)).EnsureExists("采购清单不存在");
    ...
    {
        var shoppingList = (await _unitOfWork.ShoppingLists.GetWithItemsByIdAsync(shoppingListId)).EnsureExists("采购清单不存在");
    ...
    {
        var item = (await _unitOfWork.ShoppingListItems.GetByIdAsync(itemId)).EnsureExists("清单项不存在");

        var shoppingList = (await _unitOfWork.ShoppingLists.GetByIdAsync(item.ShoppingListId)).EnsureExists("采购清单不存在");
    ...
    {
        var shoppingList = (await _unitOfWork.ShoppingLists.GetWithItemsByIdAsync(dto.ShoppingListId)).EnsureExists("采购清单不存在");
