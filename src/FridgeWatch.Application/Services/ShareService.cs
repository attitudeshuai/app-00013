...
public class ShareService : IShareService
{
    ...
    public async Task<ShareLinkDto> CreateShareLinkAsync(CreateShareLinkDto dto, int userId)
    {
        ...
        var household = await _unitOfWork.Households.GetByIdAsync(dto.HouseholdId).ThrowIfNullAsync("家庭不存在");
    ...
    {
        var shareLink = await _unitOfWork.ShareLinks.GetByIdAsync(shareLinkId).ThrowIfNullAsync("分享链接不存在");
    ...
    {
        var shareLink = await _unitOfWork.ShareLinks.GetByTokenAsync(token).ThrowIfNullAsync("分享链接不存在或已被撤销");
