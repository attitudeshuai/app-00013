...
public class ConsumptionRecordService : IConsumptionRecordService
{
    ...
    {
        var record = (await _unitOfWork.ConsumptionRecords.GetByIdAsync(id)).EnsureExists("消耗记录不存在");
    ...
    {
        var foodItem = (await _unitOfWork.FoodItems.GetByIdAsync(dto.FoodItemId)).EnsureExists("食材不存在");
    ...
    public async Task DeleteAsync(int id, int userId)
    {
        ...
        var foodItem = (await _unitOfWork.FoodItems.GetByIdAsync(record.FoodItemId)).EnsureExists("对应食材不存在");
