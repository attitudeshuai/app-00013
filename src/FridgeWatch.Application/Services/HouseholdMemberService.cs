...
public class HouseholdMemberService : IHouseholdMemberService
{
    ...
    {
        var member = await _unitOfWork.HouseholdMembers.GetByIdAsync(id).ThrowIfNullAsync("成员不存在");
    ...
    {
        var household = await _unitOfWork.Households.GetByInviteCodeAsync(inviteCode).ThrowIfNullAsync("邀请码无效");
