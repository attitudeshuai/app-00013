...
public class HouseholdMemberService : IHouseholdMemberService
{
    ...
    {
        var member = (await _unitOfWork.HouseholdMembers.GetByIdAsync(id)).EnsureExists("成员不存在");
    ...
    {
        var household = (await _unitOfWork.Households.GetByInviteCodeAsync(inviteCode)).EnsureExists("邀请码无效");
